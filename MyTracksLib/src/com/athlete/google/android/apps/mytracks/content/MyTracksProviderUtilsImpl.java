...
import com.athlete.google.android.apps.mytracks.content.Sensor.SensorDataSet;
import com.athlete.google.android.apps.mytracks.content.WaypointCreationRequest.WaypointType;
...
public class MyTracksProviderUtilsImpl implements MyTracksProviderUtils {
	...
	private static final String TAG = MyTracksProviderUtilsImpl.class
			.getSimpleName();
	...
	public Track createTrack(Cursor cursor) {
		...
		int modifiedTimeIndex = cursor
				.getColumnIndexOrThrow(TracksColumns.MODIFIEDTIME);
		int sharedWithMeIndex = cursor
				.getColumnIndexOrThrow(TracksColumns.SHAREDWITHME);
		int sharedOwnerIndex = cursor
				.getColumnIndexOrThrow(TracksColumns.SHAREDOWNER);
	...
	public void deleteAllTracks(Context context) {
		...
		/*
		 * File dir = FileUtils.getPhotoDir(); deleteDirectoryRecurse(context,
		 * dir); //THIS FUNCTION IS NOT AVAILABLE IN THE FILEUTILSS CLASS
		 */
	...
	private void deleteTrackPointsAndWaypoints(Context context, long trackId) {
		...
		if (track != null) {
			String where = TrackPointsColumns._ID + ">=? AND "
					+ TrackPointsColumns._ID + "<=?";
			String[] selectionArgs = new String[] {
					Long.toString(track.getStartId()),
					Long.toString(track.getStopId()) };
			contentResolver.delete(TrackPointsColumns.CONTENT_URI, where,
					selectionArgs);
		}
	...
	/*
	 * private void deleteDirectoryRecurse(Context context, File dir) { if
	 * (FileUtils.isDirectory(dir)) { for (File child : dir.listFiles()) {
	 * deleteDirectoryRecurse(context, child); } } if (dir.exists()) {
	 * dir.delete(); FileUtils.updateMediaScanner(context, Uri.fromFile(dir)); }
	 * }
	 */
	...
	public List<Track> getAllTracks() {
		 ...
		 * ArrayList<Track> tracks = new ArrayList<Track>(); if (cursor != null)
		 * { tracks.ensureCapacity(cursor.getCount()); if (cursor.moveToFirst())
		 * { do { tracks.add(createTrack(cursor)); } while
		 * (cursor.moveToNext()); } cursor.close(); } return tracks;
		 */
	...
	public Track getLastTrack() {
		Cursor cursor = null;
		try {
			/*String selection = TracksColumns._ID + "=(select max("
					+ TracksColumns._ID + ") from " + TracksColumns.TABLE_NAME
					+ ")";
	...
	@Override
	public Cursor getTrackCursor(String selection, String[] selectionArgs,
			String sortOrder) {
	...
	public void updateTrack(Track track) {
		contentResolver.update(TracksColumns.CONTENT_URI,
				createContentValues(track), TracksColumns._ID + "=?",
	...
	private Cursor getTrackCursor(String[] projection, String selection,
		...
		return contentResolver.query(TracksColumns.CONTENT_URI, projection,
	...
	public void deleteWaypoint(Context context,long waypointId, DescriptionGenerator descriptionGenerator) {
		...
		if (waypoint != null && waypoint.getType() == Waypoint.TYPE_STATISTICS && descriptionGenerator != null) {
			...
			if (nextWaypoint == null) {
				...
				nextWaypoint.setDescription(descriptionGenerator
						.generateWaypointDescription(nextWaypoint
								.getTripStatistics()));
	...
	public int getNextWaypointNumber(long trackId, WaypointType waypointType) {
		...
		try {
			...
			int type=waypointType.ordinal();
			String[] selectionArgs = new String[] { Long.toString(trackId),
					Integer.toString(type) };
			cursor = getWaypointCursor(projection, selection, selectionArgs,
					WaypointsColumns._ID, -1);
			if (cursor != null) {
	...
	@Override
	public Cursor getWaypointCursor(String selection, String[] selectionArgs,
			String sortOrder, int maxWaypoints) {
		return getWaypointCursor(null, selection, selectionArgs, sortOrder,
				maxWaypoints);
	}

	@Override
	public Cursor getWaypointCursor(long trackId, long minWaypointId,
			int maxWaypoints) {
		if (trackId < 0) {
			return null;
		}
		...
		if (minWaypointId >= 0) {
			selection = WaypointsColumns.TRACKID + "=? AND "
					+ WaypointsColumns._ID + ">=?";
		...
		return getWaypointCursor(null, selection, selectionArgs,
				WaypointsColumns._ID, maxWaypoints);
	}
	...
	public boolean updateWaypoint(Waypoint waypoint) {
		int rows = contentResolver.update(WaypointsColumns.CONTENT_URI,
				createContentValues(waypoint), WaypointsColumns._ID + "=?",
	...
	ContentValues createContentValues(Waypoint waypoint) {
		...
		if (tripStatistics != null) {
			...
			values.put(WaypointsColumns.MINELEVATION,
					tripStatistics.getMinElevation());
			values.put(WaypointsColumns.MAXELEVATION,
					tripStatistics.getMaxElevation());
		...
		values.put(WaypointsColumns.PHOTOURL, "");
		return values;
	}
	...
	private Waypoint getNextStatisticsWaypointAfter(Waypoint waypoint) {
		Cursor cursor = null;
		try {
			String selection = WaypointsColumns._ID + ">?  AND "
					+ WaypointsColumns.TRACKID + "=? AND "
					+ WaypointsColumns.TYPE + "=" + WaypointType.STATISTICS.ordinal();;
			String[] selectionArgs = new String[] {
					Long.toString(waypoint.getId()),
	...
	private Cursor getWaypointCursor(String[] projection, String selection,
		...
		return contentResolver.query(WaypointsColumns.CONTENT_URI, projection,
				selection, selectionArgs, sortOrder);
	}

	@Override
	public int bulkInsertTrackPoint(Location[] locations, int length,
			long trackId) {
		if (length == -1) {
		...
		return contentResolver.bulkInsert(TrackPointsColumns.CONTENT_URI,
				values);
	}
	...
	public long getFirstTrackPointId(long trackId) {
		...
		try {
			String selection = TrackPointsColumns._ID + "=(select min("
					...
					+ TrackPointsColumns.TABLE_NAME + " WHERE "
					+ TrackPointsColumns.TRACKID + "=?)";
			String[] selectionArgs = new String[] { Long.toString(trackId) };
			cursor = getTrackPointCursor(
					new String[] { TrackPointsColumns._ID }, selection,
					selectionArgs, TrackPointsColumns._ID);
			if (cursor != null && cursor.moveToFirst()) {
				return cursor.getLong(cursor
						.getColumnIndexOrThrow(TrackPointsColumns._ID));
			}
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public Location getLastValidTrackPoint(long trackId) {
		...
		String selection = TrackPointsColumns._ID + "=(select max("
				+ TrackPointsColumns._ID + ") from "
				+ TrackPointsColumns.TABLE_NAME + " WHERE "
				+ TrackPointsColumns.TRACKID + "=? AND "
				+ TrackPointsColumns.LATITUDE + "<=" + MAX_LATITUDE + ")";
		String[] selectionArgs = new String[] { Long.toString(trackId) };
		return findTrackPointBy(selection, selectionArgs);
	}
	...
	@Override
	public Cursor getTrackPointCursor(long trackId, long startTrackPointId,
	...
	public LocationIterator getTrackPointLocationIterator(final long trackId,
		...
		return new LocationIterator() {
			...
			private Cursor getCursor(long trackPointId) {
				return getTrackPointCursor(trackId, trackPointId,
						defaultCursorBatchSize, descending);
			}
	...
	private ContentValues createContentValues(Location location, long trackId) {
		...
		if (location instanceof MyTracksLocation) {
			...
			if (myTracksLocation.getSensorDataSet() != null) {
				values.put(TrackPointsColumns.SENSOR, myTracksLocation
	 ...
	 * 
	 * @param cursor
	 *            the cursor pointing to a location.
	 * @param indexes
	 *            the cached track points indexes
	 * @param location
	 *            the track point
	 */
	private void fillTrackPoint(Cursor cursor,
			CachedTrackPointsIndexes indexes, Location location) {
		location.reset();
	...
	private Location findTrackPointBy(String selection, String[] selectionArgs) {
		Cursor cursor = null;
		try {
			cursor = getTrackPointCursor(null, selection, selectionArgs,
					TrackPointsColumns._ID);
			if (cursor != null && cursor.moveToNext()) {
	...
	private Cursor getTrackPointCursor(String[] projection, String selection,
			String[] selectionArgs, String sortOrder) {
		return contentResolver.query(TrackPointsColumns.CONTENT_URI,
	...
	private static class CachedTrackPointsIndexes {
		...
		public CachedTrackPointsIndexes(Cursor cursor) {
			...
			longitudeIndex = cursor
					.getColumnIndexOrThrow(TrackPointsColumns.LONGITUDE);
			latitudeIndex = cursor
					.getColumnIndexOrThrow(TrackPointsColumns.LATITUDE);
			timeIndex = cursor.getColumnIndexOrThrow(TrackPointsColumns.TIME);
			altitudeIndex = cursor
					.getColumnIndexOrThrow(TrackPointsColumns.ALTITUDE);
			accuracyIndex = cursor
					.getColumnIndexOrThrow(TrackPointsColumns.ACCURACY);
			speedIndex = cursor.getColumnIndexOrThrow(TrackPointsColumns.SPEED);
			bearingIndex = cursor
					.getColumnIndexOrThrow(TrackPointsColumns.BEARING);
			sensorIndex = cursor
					.getColumnIndexOrThrow(TrackPointsColumns.SENSOR);
		}
	}
	 ...
	 * 
	 * @param defaultCursorBatchSize
	 *            the default cursor batch size
	 */
