...
import com.athlete.google.android.apps.mytracks.TrackEditActivity;
...
public class TrackRecordingServiceConnectionUtils {
	...
	private static final String TAG = TrackRecordingServiceConnectionUtils.class
			.getSimpleName();
				...
				if (showEditor) {
					...
					if (recordingTrackId != PreferencesUtils.RECORDING_TRACK_ID_DEFAULT) {
						Intent intent = IntentUtils
								.newIntent(context, TrackEditActivity.class)
								.putExtra(TrackEditActivity.EXTRA_TRACK_ID,
										recordingTrackId)
								.putExtra(TrackEditActivity.EXTRA_NEW_TRACK,
										true);
	...
	private static void resetRecordingState(Context context) {
		...
		if (!recordingTrackPaused) {
			PreferencesUtils.setBoolean(context,
					R.string.recording_track_paused_key,
					PreferencesUtils.RECORDING_TRACK_PAUSED_DEFAULT);
		}
	}
	...
	public static void addMarker(Context context,
		...
		if (trackRecordingService == null) {
			...
			try {
				if (trackRecordingService
						.insertWaypoint(waypointCreationRequest) != -1L) {
					Toast.makeText(context, R.string.marker_add_success,
		...
		Toast.makeText(context, R.string.marker_add_error, Toast.LENGTH_LONG)
				.show();
	}
	public static void resumeConnection(Context context,TrackRecordingServiceConnection trackRecordingServiceConnection) {
		trackRecordingServiceConnection.bindIfStarted();
		if (!isRecordingServiceRunning(context)) {
			PreferencesUtils.setLong(context, R.string.recording_track_id_key,
					PreferencesUtils.RECORDING_TRACK_ID_DEFAULT);
			PreferencesUtils.setBoolean(context,
					R.string.recording_track_paused_key, true);
		}
	}
