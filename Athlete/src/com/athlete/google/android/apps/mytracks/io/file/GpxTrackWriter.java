...
public class GpxTrackWriter implements TrackFormatWriter {
	...
	public void writeHeader() {
		 ...
		 * StringUtils.formatDateTimeIso8601(track
		 * .getTripStatistics().getStartTime()) + "</time>");
		 * 
		 * printWriter.println("<extensions>"); printWriter.println("<subtype>"
		 * + track.getActivitySubType() + "</subtype>");
		 * printWriter.println("<privacy>" + track.getPrivacy().toLowerCase() +
		 * "</privacy>"); if (track.getAcces() != null &&
		 * track.getAcces().length() > 0) {
		 * printWriter.println("<facebook_access_token>" + track.getAcces() +
		 * "</facebook_access_token>"); } printWriter.println("</extensions>");
		 * printWriter.println("</metadata>"); }
		 */
		if (printWriter != null) {
			printWriter.println("<?xml version=\"1.0\" encoding=\"UTF-8\"?>");
			...
			printWriter.println("<desc>"
					+ StringUtils.formatCData(track.getDescription())
					+ "</desc>");
			printWriter.println("</metadata>");
		}
	}
	...
	public void writeBeginTrack(Location firstLocation) {
		/*if (printWriter != null) {
			...
			// TODO: Activity types
			printWriter.println("<type>" + track.getActivityType() + "</type>");
...
*/
	...
	public void writeLocation(Location location) {
		/*if (printWriter != null) {
			if (prevTime == -1L) {
				wtiteLocations(location);
				prevTime = location.getTime();
			} else if (prevTime < location.getTime()) {
				wtiteLocations(location);
				prevTime = location.getTime();
			}
	...
	/*private void wtiteLocations(Location location) {
		printWriter.println("<trkpt " + formatLocation(location) + ">");
		printWriter.println("<ele>"
				+ ELEVATION_FORMAT.format(location.getAltitude()) + "</ele>");
		printWriter.println("<time>"
