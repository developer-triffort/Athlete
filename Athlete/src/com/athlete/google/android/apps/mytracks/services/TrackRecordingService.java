...
package com.athlete.google.android.apps.mytracks.services;

import static com.athlete.Constants.RESUME_TRACK_EXTRA_NAME;
...
import com.athlete.google.android.apps.mytracks.content.DescriptionGeneratorImpl;
import com.athlete.google.android.apps.mytracks.content.MyTracksProvider;
...
import com.athlete.google.android.apps.mytracks.content.WaypointCreationRequest;
import com.athlete.google.android.apps.mytracks.content.WaypointCreationRequest.WaypointType;
import com.athlete.google.android.apps.mytracks.services.tasks.AnnouncementPeriodicTaskFactory;
import com.athlete.google.android.apps.mytracks.services.tasks.PeriodicTask;
import com.athlete.google.android.apps.mytracks.services.tasks.PeriodicTaskExecutor;
import com.athlete.google.android.apps.mytracks.services.tasks.PeriodicTaskFactory;
import com.athlete.google.android.apps.mytracks.stats.TripStatistics;
import com.athlete.google.android.apps.mytracks.stats.TripStatisticsUpdater;
...
import com.athlete.google.android.apps.mytracks.util.TrackNameUtils;
...
public class TrackRecordingService extends Service {
	...
	private static final String TAG = TrackRecordingService.class
			.getSimpleName();
	public static final double PAUSE_LATITUDE = 100.0;
	public static final double RESUME_LATITUDE = 200.0;
	public static final int onMS = 200;
	public static final int offMS = 600;
	...
	private final OnSharedPreferenceChangeListener sharedPreferenceChangeListener = new OnSharedPreferenceChangeListener() {
		...
		public void onSharedPreferenceChanged(SharedPreferences preferences,
			...
			if (key == null
				...
				long trackId = PreferencesUtils.getLong(context,
						R.string.recording_track_id_key);
				/*
			...
			if (key == null
					|| key.equals(PreferencesUtils.getKey(context,
							R.string.announcement_frequency_key))) {
	...
	public void onCreate() {
		...
		announcementExecutor = new PeriodicTaskExecutor(this,
		...
		sharedPreferences = getSharedPreferences(Constants.SETTINGS_NAME,
		...
		if (track != null) {
			...
			if (isRecording()) {
				Log.w(TAG, "track is null, but recordingTrackId not -1L. "
						+ recordingTrackId);
	...
	public long insertWaypoint(WaypointCreationRequest waypointCreationRequest) {
		...
		if (waypointCreationRequest.getName() != null) {
			...
			int nextWaypointNumber = myTracksProviderUtils.getNextWaypointNumber(recordingTrackId, waypointCreationRequest.getType());
		...
		int type = isStatistics ? Waypoint.TYPE_STATISTICS
		...
		Waypoint waypoint = new Waypoint(name, description, category, icon,
				recordingTrackId, type, length, duration, -1L, -1L, location,
				tripStatistics);
	...
	private long startNewTrack() {
		...
		track.setCategory(PreferencesUtils.getString(this,
				R.string.default_activity_key,
				PreferencesUtils.DEFAULT_ACTIVITY_DEFAULT));
		// TODO: Activity types
		// track.setRuntype(PreferencesUtils.getString(this,
		// R.string.default_activity_key,
		// PreferencesUtils.DEFAULT_ACTIVITY_DEFAULT));
	...
	private void restartTrack(Track track) {
		...
		Waypoint waypoint = myTracksProviderUtils
				.getLastStatisticsWaypoint(recordingTrackId);
	 ...
	 * 
	 * @param trackStarted
	 *            true if track is started, false if track is resumed
	 */
	...
	private void pauseCurrentTrack() {
		...
		if (trackTripStatisticsUpdater != null && lastLocation != null)
			trackTripStatisticsUpdater.updateElevation(lastLocation
					.getAltitude());
		// Update shared preferences
		...
		if (track != null) {
			insertLocation(track, lastLocation,
					getLastValidTrackPointInCurrentSegment(track.getId()));
	 ...
	 * 
	 * @param trackStopped
	 *            true if track is stopped, false if track is paused
	 * @param trackId
	 *            the track id
	 */
	...
	private void updateRecordingState(long trackId, boolean paused) {
		recordingTrackId = trackId;
		PreferencesUtils
				.setLong(this, R.string.recording_track_id_key, trackId);
	...
	private void onLocationChangedAsync(Location location) {
		try {
			...
			Location lastValidTrackPoint = getLastValidTrackPointInCurrentSegment(track
					.getId());
			long idleTime = lastValidTrackPoint != null ? location.getTime()
					- lastValidTrackPoint.getTime() : 0L;
	 ...
	 * @param location
	 *            the location
	 * @param lastValidTrackPoint
	 *            the last valid track point, can be null
	 */
	...
	public void showNotification() {
		...
		if (isRecording() && !isPaused()) {
			Intent intent = new Intent(this, TabActivityTrack.class)
					.putExtra(TabActivityTrack.EXTRA_TRACK_ID, recordingTrackId)
					.putExtra(com.athlete.Constants.INTENT_KEY.NOTIFICATION,
							true)
					.setFlags(
			...
			NotificationCompat.Builder builder = new NotificationCompat.Builder(
					this)
					.setLights(Color.RED, onMS, offMS)
					.setContentIntent(contentIntent)
					.setContentText(
							getString(R.string.track_record_notification))
