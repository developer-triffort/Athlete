...
public class LocationUtils {
	 ...
	 * @param c2
	 *            the end of the lone segment
	 * @return the distance in m (assuming spherical earth)
	 */
	public static double distance(final Location c0, final Location c1,
			final Location c2) {
		if (c1.equals(c2)) {
	 ...
	 *            output
	 */
	public static void decimate(double tolerance,
			ArrayList<Location> locations, ArrayList<Location> decimated) {
		...
		if (n > 2) {
			...
			while (stack.size() > 0) {
				...
				for (idx = current[0] + 1; idx < current[1]; ++idx) {
					dist = LocationUtils.distance(locations.get(idx),
	 ...
	 *            the track to split
	 * @param maxPoints
	 *            maximum number of points for each piece
	...
	public static ArrayList<Track> split(Track track, int maxPoints) {
		...
		do {
            ...
            // TODO: Activity types
			//piece.setRuntype(track.getRuntype());
			...
			if (nPointsPiece >= 2) {
				...
				pieceStats.setStopTime(pieceLocations.get(nPointsPiece - 1)
						.getTime());
				result.add(piece);
			}
