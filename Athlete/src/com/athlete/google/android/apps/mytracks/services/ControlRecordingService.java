...
public class ControlRecordingService extends IntentService implements ServiceConnection {
	 ...
	 * @param intent
	 *            to be handled
	 * @param trackRecordingService
	 *            the trackRecordingService
	 */
	@VisibleForTesting
	void onHandleIntent(Intent intent, 	ITrackRecordingService trackRecordingService) {
		String action = intent.getAction();
		if (action != null) {
			try {
				/*
				 * if (action.equals(getString(R.string.track_action_start))) {
				 * trackRecordingService.startNewTrack(); } else if
				 * (action.equals(getString(R.string.track_action_end))) {
				...
				if (action.equals(getString(R.string.track_action_start))) {
					...
					trackRecordingService.endCurrentTrack();
						...
						.equals(getString(R.string.track_action_pause))) {
					trackRecordingService.pauseCurrentTrack();
				} else if (action
						.equals(getString(R.string.track_action_resume))) {
