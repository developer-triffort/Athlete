...
import com.athlete.google.android.apps.mytracks.content.MyTracksLocation;
...
import com.athlete.google.android.apps.mytracks.util.FileUtils;
...
class TrackWriterImpl implements TrackWriter {
		...
		if (track != null) {
			if (openFile()) {
				try {
					...
					if (file != null) {
						if (!file.delete()) {
							Log.w(Constants.TAG, "Failed to delete file "
	...
	protected boolean openFile() {
		...
		// filename)
		String fileName = FileUtils.buildUniqueFileName(directory,
				track.getName(), writer.getExtension());
		if (fileName == null) {
			Log.e(Constants.TAG,
	...
	protected boolean canWriteFile() {
		if (directory == null) {
			String dirName = FileUtils.buildExternalDirectoryPath(writer
					.getExtension());
		...
		if (cursor != null) {
			try {
				if (cursor.moveToFirst()) {
					// Yes, this will skip the 1st way point and that is
					// intentional
					// as the 1st points holds the stats for the current/last
					// segment.
	...
	private void writeLocations() throws InterruptedException {
		...
		class TrackWriterLocationFactory implements
				MyTracksProviderUtils.LocationFactory {
		...
		LocationIterator it = providerUtils.getTrackPointLocationIterator(track.getId(), 0, false, locationFactory);
		try {
			int pointNumber = 0;
			while (it.hasNext()) {
				...
				if (validSegment) {
					if (!segmentOpen) {
						...
						// Write the previous point, which we had previously
						// skipped
