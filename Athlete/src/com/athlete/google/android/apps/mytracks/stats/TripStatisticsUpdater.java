...
package com.athlete.google.android.apps.mytracks.stats;
...
public class TripStatisticsUpdater {
	...
	private final DoubleBuffer speedBuffer = new DoubleBuffer(
			Constants.SPEED_SMOOTHING_FACTOR);
	...
	private final DoubleBuffer distanceBuffer = new DoubleBuffer(
			Constants.DISTANCE_SMOOTHING_FACTOR);

	// A buffer of the recent grade calculations (%)
	private final DoubleBuffer gradeBuffer = new DoubleBuffer(
			Constants.GRADE_SMOOTHING_FACTOR);
	...
	public TripStatistics getTripStatistics() {
		// Take a snapshot - we don't want anyone messing with our
		// tripStatistics
	...
	public void addLocation(Location location, int minRecordingDistance) {
		if (!LocationUtils.isValidLocation(location)) {
			...
			if (location.getLatitude() == TrackRecordingService.PAUSE_LATITUDE) {
				if (lastLocation != null && lastMovingLocation != null
					...
					currentSegment.addTotalDistance(lastMovingLocation
		...
		updateSpeed(location.getTime(), location.getSpeed(),
	 ...
	 * @param speed
	 *            the speed
	 * @param lastLocationTime
	 *            the last location time
	 * @param lastLocationSpeed
	 *            the last location speed
	 */
	@VisibleForTesting
	void updateSpeed(long time, double speed, long lastLocationTime,
		...
		if (!isValidSpeed(time, speed, lastLocationTime, lastLocationSpeed)) {
			Log.d(Constants.TAG, "Invalid speed. speed: " + speed
	...
	public double updateElevation(double elevation) {
		...
		double elevationDifference = elevationBuffer.isFull() ? newAverage
				- oldAverage : 0.0;
	 ...
	 * 
	 * @param distance
	 *            the distance the user just traveled
	 * @param elevationDifference
	 *            the elevation difference between the current reading and the
	 *            previous reading
	 */
	@VisibleForTesting
	 ...
	 *            the last location speed
	 */
	private boolean isValidSpeed(long time, double speed,
		...
		return (speed < average * 10)
				&& (diff < Constants.MAX_ACCELERATION * timeDifference);
	}
}
