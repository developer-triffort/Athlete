 ...
 * 
 * One example of a broadcast message that this class is interested in, is
 * notification about the phone boot. We may want to resume a previously started
 * tracking session if the phone crashed (hopefully not), or the user decided to
 * swap the battery or some external event occurred which forced a phone reboot.
 * 
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (ACTION_BOOT_COMPLETED.equals(intent.getAction())) {
			Intent startIntent = new Intent(context,
					TrackRecordingService.class).putExtra(
					RESUME_TRACK_EXTRA_NAME, true);
			context.startService(startIntent);

			Intent removeTempFilesIntent = new Intent(context,
					RemoveTempFilesService.class);
