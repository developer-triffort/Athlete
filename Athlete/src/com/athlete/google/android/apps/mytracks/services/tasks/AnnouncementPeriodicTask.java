...
package com.athlete.google.android.apps.mytracks.services.tasks;
...
public class AnnouncementPeriodicTask implements PeriodicTask {
	...
	private static final String TAG = AnnouncementPeriodicTask.class
			.getSimpleName();
	...
	static {
		SPEECH_PARAMS.put(TextToSpeech.Engine.KEY_PARAM_UTTERANCE_ID,
				"not_used");
	}
	...
	/**
	 * Listener which updates {@link #speechAllowed} when the phone state
	 * changes.
	 */
	...
	public void start() {
		...
		speechAllowed = true;
		listenToPhoneState(phoneStateListener,
	...
	protected void onTtsReady() {
		...
		if (languageAvailability == TextToSpeech.LANG_MISSING_DATA
			...
			/*
			 * TODO: instead of using english, load the language if missing and
			 * show a toast if not supported. Not able to change the resource
			 * strings to English.
			 */
		}
	 ...
	 */
	@VisibleForTesting
	protected TextToSpeech newTextToSpeech(Context aContext,
	...
	protected String getAnnouncement(TripStatistics tripStatistics) {
		...
		boolean reportSpeed = PreferencesUtils.getBoolean(context,
				R.string.report_speed_key,
		...
		if (reportSpeed) {
			...
			rate = context.getResources().getQuantityString(speedId,
					getQuantityCount(speed), speed);
		} else {
			speed = speed == 0 ? 0.0 : 1 / speed;
			int paceId = metricUnits ? R.string.voice_pace_per_kilometer
					: R.string.voice_pace_per_mile;
			rate = context.getString(paceId,
					getAnnounceTime((long) (speed * HOUR_TO_MILLISECOND)));
		}
		...
		String totalDistance = context.getResources().getQuantityString(
				totalDistanceId, getQuantityCount(distance), distance);
	 ...
	 * quantity. If the double quantity is exactly 0, 1, or 2, then we can
	 * return these integer quantities. Otherwise, we cast the double quantity
	 * to an integer quantity. However, we need to make sure that if the casted
	 * value is 0, 1, or 2, we don't return those, instead, return the next
	 * biggest integer 3.
	 * 
	 * @param d
	 *            the double value
