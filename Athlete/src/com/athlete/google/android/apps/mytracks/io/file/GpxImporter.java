...
public class GpxImporter extends DefaultHandler {
	...
	private static final int MAX_BUFFERED_LOCATIONS = 4096;
	private static final int DEFF_SPEED = 130;
	 ...
	 * 
	 * @param inputStream
	 *            the input stream for the GPX file
	 * @param myTracksProviderUtils
	 *            my tracks provider utils
	 * @param minRecordingDistance
	 *            the min recording distance
	 * @return long[] array of track ids written to the database.
	 */
	public static long[] importGPXFile(InputStream inputStream,
			MyTracksProviderUtils myTracksProviderUtils,int minRecordingDistance) throws ParserConfigurationException,
	...
	public void rollbackUnfinishedTracks() {
		if (!isCurrentTrackFinished && track != null) {
			myTracksProviderUtils.deleteTrack(null,track.getId());
	...
	private void onTrackSegmentElementStart() {
		...
		// If not the first segment, add a pause separator if there is at least
		// one
		// location in the last segment
		if (numberOfTrackSegments > 1 && lastLocationInSegment != null) {
			insertPoint(createNewLocation(TrackRecordingService.PAUSE_LATITUDE,
					0.0, lastLocationInSegment.getTime()));
		}
		location = null;
	...
	private void onTrackPointElementEnd() throws SAXException {
		...
		if (numberOfTrackSegments > 1 && lastLocationInSegment == null) {
			// If not the first segment, add a resume separator before adding
			// the
			// first location.
			insertPoint(createNewLocation(TrackRecordingService.RESUME_LATITUDE, 0.0,location.getTime()));
		}
	...
	private void onTimeElementEnd() throws SAXException {
		...
		try {
			if (content.trim().length() > 0)
				...
				// Server get GPX file without time
				time = 0;
				location.setSpeed(DEFF_SPEED);
			}
	...
	private void flushPoints() {
		...
		myTracksProviderUtils.bulkInsertTrackPoint(bufferedLocations,
				numBufferedLocations, track.getId());
