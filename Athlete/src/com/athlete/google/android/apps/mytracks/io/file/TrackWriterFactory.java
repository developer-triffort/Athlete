...
public class TrackWriterFactory {
	 ...
	 * 
	 * @param context
	 *            the context in which the track will be read
	 * @param providerUtils
	 *            the data provider utils to read the track with
	 * @param trackId
	 *            the ID of the track to be written
	 * @param format
	 *            the output format to write in
	 * @return the new track writer
	 */
	public static TrackWriter newWriter(Context context,
			MyTracksProviderUtils providerUtils, long trackId,
			TrackFileFormat format) {
		Track track = providerUtils.getTrack(trackId);
		if (track == null) {
			Log.d(Constants.TAG, "No track for " + trackId);
			return null;
		}
