...
import com.athlete.google.android.apps.mytracks.services.MyTracksLocationManager;
...
public class DataSource {
	// 5 minutes
	private static final int NETWORK_PROVIDER_MIN_TIME = 5 * 60 * 1000;
	...
	public void registerLocationListener(LocationListener listener) {
		...
		try {
			myTracksLocationManager.requestLocationUpdates(
					...
					NETWORK_PROVIDER_MIN_TIME, 0, listener);
	...
	public Location getLastKnownLocation() {
		Location location = myTracksLocationManager
				.getLastKnownLocation(LocationManager.GPS_PROVIDER);
		if (!isLocationRecent(location)) {
			// Try network location
			location = myTracksLocationManager
					.getLastKnownLocation(LocationManager.NETWORK_PROVIDER);
			String toast;
			if (isLocationRecent(location)) {
				toast = context
						.getString(R.string.my_location_approximate_location);
			} else {
				String setting = context
						.getString(GoogleLocationUtils.isAvailable(context) ? R.string.gps_google_location_settings
