...
public class DynamicSpeedTrackPathDescriptor implements TrackPathDescriptor,
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
			String key) {
		if (PreferencesUtils.getKey(context,
				R.string.track_color_mode_percentage_key).equals(key)) {
			speedMargin = PreferencesUtils.getInt(context,
	...
	public DynamicSpeedTrackPathDescriptor(Context context) {
		...
		speedMargin = PreferencesUtils.getInt(context,
				R.string.track_color_mode_percentage_key,
				PreferencesUtils.TRACK_COLOR_MODE_PERCENTAGE_DEFAULT);
		context.getSharedPreferences(Constants.SETTINGS_NAME,
	...
	public boolean updateState() {
		long selectedTrackId = PreferencesUtils.getLong(context,
				R.string.selected_track_id_key);
		if (selectedTrackId == PreferencesUtils.SELECTED_TRACK_ID_DEFAULT) {
			Log.d(Constants.TAG, "No selected track id.");
			return false;
		}
		Track track = MyTracksProviderUtils.Factory.get(context).getTrack(
				selectedTrackId);
		if (track == null) {
			Log.d(Constants.TAG, "No track for " + selectedTrackId);
			return false;
		}
		TripStatistics tripStatistics = track.getTripStatistics();
		double newAverageMovingSpeed = (int) Math.floor(tripStatistics
				.getAverageMovingSpeed() * UnitConversions.MS_TO_KMH);
	 ...
	 */
	@VisibleForTesting
	boolean isDifferenceSignificant(double oldAverageMovingSpeed,
			double newAverageMovingSpeed) {
		if (oldAverageMovingSpeed == 0) {
			return newAverageMovingSpeed != 0;
		}

		// Here, both oldAverageMovingSpeed and newAverageMovingSpeed are not
		// zero.
		double maxValue = Math
				.max(oldAverageMovingSpeed, newAverageMovingSpeed);
		double differencePercentage = Math.abs(oldAverageMovingSpeed
				- newAverageMovingSpeed)
				/ maxValue * 100.0;
