...
import static com.athlete.Constants.MAX_DISPLAYED_WAYPOINTS_POINTS;
import static com.athlete.Constants.MAX_LOCATION_AGE_MS;
import static com.athlete.Constants.MAX_NETWORK_AGE_MS;
import static com.athlete.Constants.TARGET_DISPLAYED_TRACK_POINTS;
...
import com.athlete.google.android.apps.mytracks.content.TrackDataListener.LocationState;
...
public class TrackDataHub implements DataSourceListener {
	...
	public static synchronized TrackDataHub newInstance(Context context) {
		return new TrackDataHub(context, new TrackDataManager(),MyTracksProviderUtils.Factory.get(context),
	 ...
	 * @param context
	 *            the context
	 * @param trackDataManager
	 *            the track data manager
	 * @param myTracksProviderUtils
	 *            the my tracks provider utils
	 * @param targetNumPoints
	 ...
	 * 
	 * @param trackDataListener
	 *            the track data listener
	 * @param trackDataTypes
	 *            the track data types
	 */
	 ...
	 *            the track data listener
	 */
	public void unregisterTrackDataListener(
			final TrackDataListener trackDataListener) {
		runInHanderThread(new Runnable() {
			@Override
			public void run() {
		...
		runInHanderThread(new Runnable() {
			...
			public void run() {
				notifyLocationChanged(location, false,
						trackDataManager.getListeners(TrackDataType.LOCATION));
			}
		});
	}
		...
		runInHanderThread(new Runnable() {
			...
			public void run() {
				...
				if (key == null
					...
					recordingTrackPaused = PreferencesUtils.getBoolean(context,R.string.recording_track_paused_key,
							PreferencesUtils.RECORDING_TRACK_PAUSED_DEFAULT);
				}
				if (key == null
						|| key.equals(PreferencesUtils.getKey(context,R.string.metric_units_key))) {
					metricUnits = PreferencesUtils.getBoolean(context,R.string.metric_units_key,
					...
					if (key != null) {
						for (TrackDataListener trackDataListener : trackDataManager.getListeners(TrackDataType.PREFERENCE)) {
							if (trackDataListener
									.onMetricUnitsChanged(metricUnits)) {
								loadDataForListener(trackDataListener);
							}
						}
					}
				...
				if (key == null
					...
					reportSpeed = PreferencesUtils.getBoolean(context,R.string.report_speed_key,
				...
				if (key == null
					...
					minRecordingDistance = PreferencesUtils.getInt(context,	R.string.min_recording_distance_key,
							PreferencesUtils.MIN_RECORDING_DISTANCE_DEFAULT);
					if (key != null) {
						for (TrackDataListener trackDataListener : trackDataManager	.getListeners(TrackDataType.PREFERENCE)) {
							if (trackDataListener
									.onMinRecordingDistanceChanged(minRecordingDistance)) {
								loadDataForListener(trackDataListener);
							}
						}
					}
	...
	public void notifyLocationChanged(final Location location) {
		runInHanderThread(new Runnable() {
			@Override
			public void run() {
				notifyLocationChanged(location, false,trackDataManager.getListeners(TrackDataType.LOCATION));
			}
		});
	}
			...
			public void run() {
				...
				if (lastSeenLocation != null) {
					...
					if (now - lastDeclinationUpdate > ONE_HOUR) {
						...
						if (timestamp == 0) {
							// Hack for Samsung phones which don't populate the
							// time field
							timestamp = now;
						}
	...
	private void loadDataForAll() {
		...
		for (TrackDataListener trackDataListener : trackDataManager
				.getListeners(TrackDataType.PREFERENCE)) {
		...
		for (TrackDataListener listener : trackDataManager
				.getListeners(TrackDataType.SAMPLED_IN_TRACK_POINTS_TABLE)) {
		...
		notifyTrackPointsTableUpdate(
				...
				trackDataManager
						.getListeners(TrackDataType.SAMPLED_IN_TRACK_POINTS_TABLE),
				trackDataManager
						.getListeners(TrackDataType.SAMPLED_OUT_TRACK_POINTS_TABLE));

		if (lastSeenLocation != null) {
			notifyLocationChanged(lastSeenLocation, true,
					trackDataManager.getListeners(TrackDataType.LOCATION));
		} else {
			notifyLocationStateChanged(trackDataManager
					.getListeners(TrackDataType.LOCATION));
		}
		notifyHeadingChange(trackDataManager
				.getListeners(TrackDataType.HEADING));
	}
	...
	private void loadDataForListener(TrackDataListener trackDataListener) {
		Set<TrackDataListener> trackDataListeners = Collections
				.singleton(trackDataListener);
		EnumSet<TrackDataType> trackDataTypes = trackDataManager
				.getTrackDataTypes(trackDataListener);
		...
		if (hasSampledIn || hasSampledOut) {
			...
			notifyTrackPointsTableUpdate(isOnlyListener, sampledInListeners,
					sampledOutListeners);
		}
	...
	/**
	 * Notifies selected track changed. To be run in the {@link #handler}
	 * thread.
	 * 
	 * @param trackDataListeners
	 *            the track data listeners to notify
	 */
	 ...
	 * thread.
	 * 
	 * @param updateSamplingState
	 *            true to update the sampling state
	 * @param sampledInListeners
	 *            the sampled-in listeners
	 * @param sampledOutListeners
	 *            the sampled-out listeners
	 */
	private void notifyTrackPointsTableUpdate(boolean updateSamplingState,
			Set<TrackDataListener> sampledInListeners,
		...
		LocationIterator iterator = myTracksProviderUtils
				.getTrackPointLocationIterator(selectedTrackId,
						localLastSeenLocationId + 1, false,// locationFactory
		...
		while (iterator.hasNext()) {
			...
			if (!LocationUtils.isValidLocation(location)) {
				...
				// Also include the last point if the selected track is not
				// recording.
				if (includeNextPoint
						|| (localNumLoadedPoints % samplingFrequency == 0)
	...
	/**
	 * Notifies location state changed. To be run in the {@link #handler}
	 * thread.
	 * 
	 * @param trackDataListeners
	 *            the track data listeners to notify
	 */
	private void notifyLocationStateChanged(
	 ...
	 * @param location
	 *            the location
	 * @param notifyLocationStateChange
	 *            true to always notify location state change
	 * @param trackDataListeners
	 *            the track data listeners to notify
	 */
	private void notifyLocationChanged(Location location,
			boolean notifyLocationStateChange,
			Set<TrackDataListener> trackDataListeners) {
		if (location == null) {
			return;
		}
		...
		if (location.getProvider().equals(LocationManager.GPS_PROVIDER)) {
			...
			hasFix = false;
			hasGoodFix = false;
			lastSeenLocation = isLocationOld(location, now, MAX_NETWORK_AGE_MS) ? null
					: location;
		}
	 ...
	 * 
	 * @param lastSeenLocation
	 *            value of lastSeenLocation
	 */
	@VisibleForTesting
